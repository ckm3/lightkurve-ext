...
def cache_obsid_path(
    ...
    if isinstance(dir_path, str) or isinstance(dir_path, Path):
        tqdm.write("Scanning available light curve products in path: {}".format(dir_path))
        output_list = []
        ...
        for file_path in tqdm(dir_path.rglob("*.fits")):
            ...
            if len(output_list) % 100000 == 0:
                write_tess_lc_database(cache_dir, output_list)
                tqdm.write(f"Successfully cache {len(output_list)} light curve files in {dir_path}")
                logger.info(f"Successfully cache {len(output_list)} light curve files in {dir_path}")
                output_list = []
        write_tess_lc_database(cache_dir, output_list)
        tqdm.write(f"Successfully cache {len(output_list)} light curve files in {dir_path}")
        logger.info(f"Successfully cache {len(output_list)} light curve files in {dir_path}")
        ...
        tqdm.write(f"Scanning available light curve products in a given list with length {len(dir_path)}")
        output_list = []
        for file_path in tqdm(map(Path, dir_path)):
            file_name = file_path.name
            SPOC_match = re.match(
                r"tess\d{13}-s(\d{4})-(\d{16})-\d{4}-[xsab]_lc.fits*", file_name
            )
        ...
        raise TypeError(
            "dir_path must be a str or Path object or str or Path object or list of fits file paths"
        )
