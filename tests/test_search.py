...
def test_LightCruveDirectory():
    ...
    lc_dir = lkx.LightCurveDirectory(
        './tests/data/TESS', use_cache=True)
    assert lc_dir.directories == [
        Path('./tests/data/TESS')]

    # test a list of directories
    lc_dir = lkx.LightCurveDirectory(
        ['./tests/data/TESS',
            './tests/data/HLSP'], use_cache=True)
    assert lc_dir.directories == set(
        [Path('./tests/data/TESS'),
        Path('./tests/data/HLSP')])

    # test scan a directory
    lc_dir = lkx.LightCurveDirectory(
        './tests/data/TESS', use_cache=False, scan_dir=True)
    assert len(lc_dir.obsid_path_dicts) > 0

    # test save cache
    lc_dir = lkx.LightCurveDirectory(
        './tests/data/TESS', use_cache=False, scan_dir=True, dump_scan_results=True)
    cache_path = Path.home() / '.lightkurve_ext-cache' / hashlib.md5((Path('./tests/data/TESS').as_posix()
